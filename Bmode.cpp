...
int main(){
    ...
    // Define BmodeClass pointer named currentScanline; point to scanlineHead
    BmodeClass *currentScanline = scanlineHead;

    // Create new dataBuffer pointer pointing to dataHead;
    dataBuffer *curData = dataHead;

    for (int i = 1; i < parameters->getNumScanline(); i++){ // Loop from 1 to numScanline
        curData = curData->next; // Go to next data pointer
        currentScanline->next = new BmodeClass(parameters, curData, i); // Define next scanline pointer
        currentScanline = currentScanline->next; // Go to next scanline pointer
        currentScanline->next = NULL; // Set new "next" equal to NULL
    } 
